import { createMultiStyleConfigHelpers, mergeThemeOverride, } from '@chakra-ui/react';
import { anatomy } from '@chakra-ui/theme-tools';
import { omit, pick } from 'lodash';
import { Input } from './Input';
import { comboboxParts, SingleSelect } from './SingleSelect';
export const parts = anatomy('multiselect').parts(...comboboxParts.keys, 'field', 'tag', 'tagIcon', 'chevron', 'fieldwrapper', 'itemContainer');
const { definePartsStyle, defineMultiStyleConfig } = createMultiStyleConfigHelpers(parts.keys);
const baseStyle = definePartsStyle((props) => {
    const { isFocused, isEmpty } = props;
    const comboboxBaseStyle = pick(SingleSelect.baseStyle?.(props), comboboxParts.keys);
    return {
        ...comboboxBaseStyle,
        itemContainer: {
            display: 'inline-flex',
...
const variantOutline = definePartsStyle((props) => {
    const comboboxVariantOutline = pick(SingleSelect.variants?.outline(props), comboboxParts.keys);
...
const sizes = {
    sm: definePartsStyle(mergeThemeOverride(omit(SingleSelect.sizes?.sm, ['field', 'icon']), {
        itemContainer: {
    ...
    md: definePartsStyle(mergeThemeOverride(omit(SingleSelect.sizes?.md, ['field', 'icon']), {
        itemContainer: {
            // Padding for dropdown toggle.
