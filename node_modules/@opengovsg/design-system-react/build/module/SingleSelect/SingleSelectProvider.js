...
export const SingleSelectProvider = ({ items: rawItems, value, onChange, name, filter = defaultFilter, nothingFoundLabel = 'No matching results', placeholder: placeholderProp, clearButtonLabel = 'Clear selection', isClearable = true, isSearchable = true, initialIsOpen, isInvalid: isInvalidProp, isReadOnly: isReadOnlyProp, isDisabled: isDisabledProp, isRequired: isRequiredProp, children, inputAria, colorScheme, size: _size, comboboxProps = {}, }) => {
    const theme = useTheme();
    // Required in case size is set in theme, we should respect the one set in theme.
    const size = useMemo(() => (_size ?? theme?.components?.SingleSelect?.defaultProps?.size ?? 'md'), [_size, theme?.components?.SingleSelect?.defaultProps?.size]);
    ...
    const getFilteredItems = useCallback((filterValue) => filterValue ? filter(items, filterValue) : items, [filter, items]);
    const [filteredItems, setFilteredItems] = useState(getFilteredItems(comboboxProps.initialInputValue ?? comboboxProps.inputValue));
    const memoizedSelectedItem = useMemo(() => {
        return getItemByValue(value)?.item ?? null;
    }, [getItemByValue, value]);
    const resetItems = useCallback(() => setFilteredItems(getFilteredItems()), [getFilteredItems]);
    const virtualListRef = useRef(null);
    const inputRef = useRef(null);
    const { toggleMenu, closeMenu, isOpen, getLabelProps, getMenuProps, getInputProps, getItemProps, getToggleButtonProps, highlightedIndex, selectItem, selectedItem, inputValue, setInputValue, } = useCombobox({
        labelId: `${name}-label`,
        inputId: name,
        defaultInputValue: '',
